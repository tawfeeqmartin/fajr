...
// ─── GOD RAY CONE ─────────────────────────────────────────────────────────────
// Mesh-based volumetric: inverted cone from above, Fresnel edge glow, additive blend
const _godRayVert = `
  ...
  void main() {
    ...
    vDistFromApex = clamp((position.y + coneHeight * 0.5) / coneHeight, 0.0, 1.0);
...
const _godRayFrag = `
  ...
  void main() {
    ...
    float shimmer = 0.92 + 0.08 * sin(vWorldPos.y * 3.0 + uTime * 0.4);
    float alpha = edgeFade * distFade * maxOpacity * shimmer;
...
const godRayMat = new THREE.ShaderMaterial({
  uniforms: {
    ...
    anglePower:  { value: 3.5 },
    maxOpacity:  { value: 0.06 },
    uTime:       { value: 0 },
    coneHeight:  { value: 12.0 },
  },
  vertexShader: _godRayVert,
  fragmentShader: _godRayFrag,
  transparent: true,
  blending: THREE.AdditiveBlending,
  depthWrite: false,
  side: THREE.DoubleSide,
});
// Cone: radiusTop (base at light), radiusBottom (at cube), height, segments, open-ended
const godRayCone = new THREE.Mesh(
  new THREE.ConeGeometry(3.5, 12, 32, 1, true),
  godRayMat
);
godRayCone.position.set(0, 6.5, 0); // centered above cube, apex at y=12.5, base near floor
godRayCone.rotation.x = Math.PI;     // flip so wide end is up (light source)
scene.add(godRayCone);

// ─── DUST MOTES ───────────────────────────────────────────────────────────────
const DUST_COUNT = _isMobile ? 180 : 300;
const _dustPositions = new Float32Array(DUST_COUNT * 3);
const _dustSizes = new Float32Array(DUST_COUNT);
const _dustSpeeds = new Float32Array(DUST_COUNT); // drift speed per mote
const _dustRadius = 4.0;  // horizontal spread
const _dustMinY = -0.5;
const _dustMaxY = 11.0;

for (let i = 0; i < DUST_COUNT; i++) {
  // Random position in cylinder around cube
  const angle = Math.random() * Math.PI * 2;
  const r = Math.random() * _dustRadius;
  _dustPositions[i * 3]     = Math.cos(angle) * r;
  _dustPositions[i * 3 + 1] = _dustMinY + Math.random() * (_dustMaxY - _dustMinY);
  _dustPositions[i * 3 + 2] = Math.sin(angle) * r;
  _dustSizes[i] = 1.5 + Math.random() * 3.5;
  _dustSpeeds[i] = 0.3 + Math.random() * 0.7;
}
...
const _dustVert = `
  ...
  void main() {
    // Gentle drift: slow sine on x/z, very slow descent on y
    vec3 pos = position;
    pos.x += sin(uTime * 0.15 * aSpeed + position.z * 2.0) * 0.3;
    pos.z += cos(uTime * 0.12 * aSpeed + position.x * 2.0) * 0.3;
    pos.y -= mod(uTime * 0.08 * aSpeed, ${(_dustMaxY - _dustMinY).toFixed(1)});
    // Wrap vertically
    pos.y = ${_dustMinY.toFixed(1)} + mod(pos.y - ${_dustMinY.toFixed(1)}, ${(_dustMaxY - _dustMinY).toFixed(1)});
    ...
    float scatter = pow(max(dot(toCamera, -toLight), 0.0), 4.0);
    // Brighter near light axis (vertical center)
    float axisDist = length(pos.xz) / ${_dustRadius.toFixed(1)};
    float inBeam = 1.0 - smoothstep(0.0, 1.0, axisDist);
    vBrightness = mix(0.04, 1.0, scatter * inBeam * 0.7 + inBeam * 0.3);
  }
`;
const _dustFrag = `
  ...
  void main() {
    ...
    alpha *= vBrightness * 0.5;
    gl_FragColor = vec4(dustColor, alpha);
  }
`;
const dustGeo = new THREE.BufferGeometry();
dustGeo.setAttribute('position', new THREE.BufferAttribute(_dustPositions, 3));
dustGeo.setAttribute('aSize', new THREE.BufferAttribute(_dustSizes, 1));
dustGeo.setAttribute('aSpeed', new THREE.BufferAttribute(_dustSpeeds, 1));

const dustMat = new THREE.ShaderMaterial({
  uniforms: {
    uTime:     { value: 0 },
    lightPos:  { value: new THREE.Vector3(0, 13, 0) }, // above the cone
    dustColor: { value: new THREE.Color(1.0, 0.95, 0.88) },
  },
  vertexShader: _dustVert,
  fragmentShader: _dustFrag,
  transparent: true,
  blending: THREE.AdditiveBlending,
  depthWrite: false,
});

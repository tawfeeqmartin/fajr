...
| **v1.7.21** | 2026-05-03 | "Best guess" framing surface — `prayerTimes()` returns now carry `location.madhab` + `location.madhabSource`, an `applied: {method, madhab, elevationMin}` summary, and a turn-key `disclaimer` user-facing string. README adds "How to think about fajr's outputs" stance section. Resolves #81. | **Yes** — apps can drive verify-this-default UX off `location.madhabSource === 'method-implied'` (show "verify your madhab" prompt for Hanafi-majority countries on adhan's Shafi'i default), `location.elevationSource === 'city-registry'` (show elevation-stance tooltip), and surface `disclaimer` verbatim in long-press / "Why this time?" sheets. |
| **v1.7.22** | 2026-05-03 | Country→madhab dispatch fix (#83). v1.7.21's `madhab` field always reported `'shafii'` because adhan presets leak. v1.7.22 adds explicit `COUNTRY_MADHAB` table — Pakistan/Bangladesh/Türkiye/Albania/Bosnia/Kosovo/Macedonia/Afghanistan/India/Central-Asia all now correctly return `madhab: 'hanafi'`. **Asr times shift by 30-60 min** in those countries (2× shadow vs 1× shadow). | **Yes** — apps can now use `r.location.madhab === 'hanafi'` to drive Hanafi-specific UX (e.g. hide jam' / qasr-prayer-combination section since Hanafis don't combine prayers). The verify-this-default UX from v1.7.21 still applies for `madhabSource === 'method-implied'` (mixed-madhab countries). |

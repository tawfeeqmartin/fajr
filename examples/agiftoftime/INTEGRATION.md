...
| **v1.7.21** | 2026-05-03 | "Best guess" framing surface — `prayerTimes()` returns now carry `location.madhab` + `location.madhabSource`, an `applied: {method, madhab, elevationMin}` summary, and a turn-key `disclaimer` user-facing string. README adds "How to think about fajr's outputs" stance section. Resolves #81. | **Yes** — apps can drive verify-this-default UX off `location.madhabSource === 'method-implied'`, `location.elevationSource === 'city-registry'` (show elevation-stance tooltip), and surface `disclaimer` verbatim in long-press / "Why this time?" sheets. |
| **v1.7.22** | 2026-05-03 | Country→Asr-school convention metadata fix (#83/#85). v1.7.21's `madhab` field always reported `'shafii'` because adhan presets leak. v1.7.22 adds explicit `COUNTRY_MADHAB` table — Pakistan/Bangladesh/Türkiye/Albania/Bosnia/Kosovo/Macedonia/Afghanistan/India/Central-Asia now report `location.madhab: 'hanafi'` as Hanafi-vs-standard Asr convention metadata, while `applied.asrSchool` reports the actual Asr formula used. **Asr times do not silently shift** from metadata alone. | **Yes** — apps can use `r.location.madhab === 'hanafi'` to preselect Hanafi UX/settings, but should read `r.applied.asrSchool` before claiming the returned Asr is Hanafi 2× shadow. Test long-press/provenance copy for cases where `location.madhab !== r.applied.madhab`; fajr emits an `Asr-school advisory` in `notes[]` for that mismatch. |

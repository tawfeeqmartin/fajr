...
function updateNightMode() {
    ...
    // Darken ring materials aggressively
    ringMeshes.forEach((mesh, i) => {
        mesh.material.color.copy(origRingColors[i]).lerp(nightCol, nightBlend * 0.92);
    });
    ...
    // Soft lume — large area elements (prayer arcs)
    softLumeMeshes.forEach(m => {
        m.material.emissiveIntensity = nightBlend * 0.4;
    });

    // Bloom
    bloomPass.strength = nightBlend * 0.9;
    bloomPass.threshold = 0.90 - nightBlend * 0.02;

    // Lighting dims heavily
    ambLight.intensity = 0.15 * (1 - nightBlend * 0.92);
    keyLight.intensity = 3.5 * (1 - nightBlend * 0.92);
    fillLight.intensity = 0.5 * (1 - nightBlend * 0.88);
    topLight.intensity = 0.35 * (1 - nightBlend * 0.85);

    // Environment dims heavily
    if (scene.environment) {
        scene.environmentIntensity = 1.6 * (1 - nightBlend * 0.82);
    }

...
// Live presets — tweakable via GUI panel, initialized from startup or localStorage
const _saved = (() => {
    try { const s = localStorage.getItem('tawafgl_presets'); return s ? JSON.parse(s) : null; } catch { return null; }
})();
const PRESETS = {
    night:   { ...STARTUP_PRESET.night,   ...(_saved?.night || {}) },
    day:     { ...STARTUP_PRESET.day,     ...(_saved?.day || {}) },
    shared:  { ...STARTUP_PRESET.shared,  ...(_saved?.shared || {}) },
    turrell: { ...STARTUP_PRESET.turrell, ...(_saved?.turrell || {}) },
};
if (_saved) console.log('[Presets] Loaded saved defaults from localStorage');
...
function renderGroupBlurred(captureGroup) {
    ...
    const tintColor = new THREE.Color().setHSL(palette.h / 360, (palette.s * 0.8) / 100, (palette.l * 1.4) / 100);

    bokehMat.uniforms.uResolution.value.set(W, H);
    bokehMat.uniforms.uTintColor.value.copy(tintColor);
...
sharedFolder.add({ rebuild() { if (artwork) { disposeAllTraceLayers(); buildTraceGeometry(artwork); applyDayNight(); } } }, 'rebuild').name('⟳ Rebuild Traces');

const turrellFolder = gui.addFolder('Turrell BG');
turrellFolder.add(PRESETS.turrell, 'glowIntensity', 0, 1.0, 0.01).name('Glow');
turrellFolder.add(PRESETS.turrell, 'blurPasses', 1, 12, 1).name('Blur Passes');
turrellFolder.add(PRESETS.turrell, 'blurScale', 0.1, 3.0, 0.05).name('Blur Scale');
turrellFolder.add(PRESETS.turrell, 'tintStrength', 0, 2.0, 0.05).name('Tint');
turrellFolder.add(PRESETS.turrell, 'fadeTarget', 0, 1.0, 0.01).name('Fade Target');
turrellFolder.add(PRESETS.turrell, 'fadeRate', 0.0001, 0.005, 0.0001).name('Fade Rate');

const devFolder = gui.addFolder('Dev');
const nightToggle = { mode: 'Auto' };
devFolder.add(nightToggle, 'mode', ['Auto', 'Night', 'Day']).name('Day/Night').onChange(v => {
    console.log('[Dev] Day/Night toggled to:', v, '→ forceNight:', v === 'Night' ? true : v === 'Day' ? false : null);
    forceNight = v === 'Night' ? true : v === 'Day' ? false : null;
    lastPrayerPeriod = null; // force trace rebuild
    if (artwork) { disposeAllTraceLayers(); buildTraceGeometry(artwork); }
    applyDayNight();
});

devFolder.add({ resetPresets() {
    Object.assign(PRESETS.night, STARTUP_PRESET.night);
    Object.assign(PRESETS.day, STARTUP_PRESET.day);
    Object.assign(PRESETS.shared, STARTUP_PRESET.shared);
    Object.assign(PRESETS.turrell, STARTUP_PRESET.turrell);
    gui.controllersRecursive().forEach(c => c.updateDisplay());
    if (artwork) { disposeAllTraceLayers(); buildTraceGeometry(artwork); }
    applyDayNight();
    console.log('[Dev] Presets reset to startup values');
} }, 'resetPresets').name('⟳ Reset Presets');

devFolder.add({ saveDefaults() {
    const snapshot = {
        night: { ...PRESETS.night },
        day: { ...PRESETS.day },
        shared: { ...PRESETS.shared },
        turrell: { ...PRESETS.turrell },
    };
    localStorage.setItem('tawafgl_presets', JSON.stringify(snapshot));
    console.log('[Dev] Defaults saved to localStorage:', snapshot);
    // Flash the button text briefly to confirm
    const btn = document.querySelector('[class*="save"]') || document.querySelector('.lil-gui .title:last-of-type');
    // Also copy to clipboard for easy pasting
    navigator.clipboard.writeText(JSON.stringify(snapshot, null, 2)).then(() => {
        console.log('[Dev] Preset JSON also copied to clipboard');
    }).catch(() => {});
    alert('Defaults saved! (also copied to clipboard)');
} }, 'saveDefaults').name('💾 Save Defaults');

devFolder.add({ clearSaved() {
    localStorage.removeItem('tawafgl_presets');
    Object.assign(PRESETS.night, STARTUP_PRESET.night);
    Object.assign(PRESETS.day, STARTUP_PRESET.day);
    Object.assign(PRESETS.shared, STARTUP_PRESET.shared);
    Object.assign(PRESETS.turrell, STARTUP_PRESET.turrell);
    ...
    if (artwork) { disposeAllTraceLayers(); buildTraceGeometry(artwork); }
    applyDayNight();
    console.log('[Dev] Cleared saved defaults, reverted to STARTUP_PRESET');
} }, 'clearSaved').name('🗑 Clear Saved');

gui.close(); // start collapsed

// Expose for lookdev console access
window._PRESETS = PRESETS;
window._rebuild = () => { if (artwork) { disposeAllTraceLayers(); buildTraceGeometry(artwork); applyDayNight(); } };
window._applyDN = () => applyDayNight();
window._setNight = (v) => { forceNight = v; applyDayNight(); };
window._epicycleDebug = () => ({
    hour: currentEpicycleHour,
    time: epicycleTime,
    trailCount: epicycleTrailCount,
    paused: epicyclePaused,
    coeffCount: hourDFTs[currentEpicycleHour]?.length || 0,
    trailVisible: epicycleTrailLine.visible,
    armVisible: epicycleArmLine.visible,
    circlesVisible: epicycleCircles.filter(c => c.line.visible).length,
    hrDotPos: [hrDot.posArr[0], hrDot.posArr[1]],
    radius: radius,
});
...
window._kaabaDFT = kaabaDFT;
window._kaabaSampled = _kaaba_sampled;
console.log('Kaaba DFT:', kaabaDFT.length, 'coeffs, top amp:', kaabaDFT[0]?.amp, 'sampled pts:', _kaaba_sampled.length);
...
function updateClockHands(now, night, blending) {
    ...
    for (let i = 0; i < epiDrawCount; i++) {
        ...
        const _tc = new THREE.Color().setHSL(baseH / 360, epiS, epiL);
        epicycleTrailColors[i * 4]     = _tc.r * brightness;
        epicycleTrailColors[i * 4 + 1] = _tc.g * brightness;
        epicycleTrailColors[i * 4 + 2] = _tc.b * brightness;
    ...
    // Copy positions from main trail, sit at z=-0.005
    for (let i = 0; i < epiDrawCount; i++) {
        kaabaGlowPositions[i * 3]     = epicycleTrailPositions[i * 3];
        kaabaGlowPositions[i * 3 + 1] = epicycleTrailPositions[i * 3 + 1];
    ...
    window._epiDebug = { trailCount: epicycleTrailCount, drawCount: epiDrawCount, penX: penWorldX, penY: penWorldY, hx, hy };

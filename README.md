...
> **Status — v1.x public beta.** fajr is in active development with frequent releases. Calculation accuracy is benchmarked against multiple references (see [CALIBRATION.md](CALIBRATION.md)). Production use is welcome — please file regression reports via issues — but downstream consumers should pin to a tested version and review release notes before bumping. Latest release: v1.7.19. See [CHANGELOG.md](CHANGELOG.md) for the full v1.x release log; [CALIBRATION.md](CALIBRATION.md) for accuracy + scholarly methodology.

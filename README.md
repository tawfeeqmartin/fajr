...
2. **Check your settings** — madhab default, elevation correction, calculation method are all auto-dispatched and may not match your community's practice. `location.madhab` reports the likely local/user madhab (for example Hanafi in Pakistan / Bangladesh / Türkiye / Albania), while `applied.asrSchool` reports the Asr formula actually used by the calculation. In v1.7.22 these are deliberately separate: country-level Hanafi metadata does **not** silently shift Asr to 2× shadow until caller override / source-specific verification lands in [#40](https://github.com/tawfeeqmartin/fajr/issues/40). Saudi Arabia institutionally declines elevation correction for jama'ah unity; UAE/JAKIM apply it. Pass `elevation: 0` to opt out of fajr's auto-applied elevation correction.
...
- **`location` field on every `prayerTimes()` / `dayTimes()` return value** — always populated, carries `{ city, country, timezone, elevation, madhab, methodSource, madhabSource, elevationSource }`. `methodSource ∈ 'caller-explicit' | 'city-institutional' | 'country-default' | 'fallback'`; `madhabSource ∈ 'caller-explicit' | 'country-default' | 'method-implied'`; `elevationSource ∈ 'caller-explicit' | 'city-registry' | 'default-zero'`. `location.madhab` is local/user metadata; `applied.asrSchool` is the calculation-facing Asr formula. Apps can render "you are in Cape Town" without a separate reverse-geocode call, and explain "Why is my Fajr at this time?" by surfacing the source enums.

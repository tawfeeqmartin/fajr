...
Source       | n   | WMAE | Δ vs v1.7.15
Aladhan      | 130 | 1.22 | unchanged (region-bounded targeting)
Mawaqit      |  25 | 0.88 | -42% (Morocco Dhuhr +5 closed -4.80 min bias)
JAKIM        |  30 | 0.45 | -22% (Dhuhr +2 / Asr +1 closed both biases)
Diyanet      |  30 | 0.50 | unchanged (region-bounded targeting)
```

Train WMAE: **0.9757** (broke the 1-minute barrier in v1.7.16; was 1.0668 at
v1.7.15, 0.6814 at v1.4.4, 1.0394 at v1.4.1, 1.2472 at v1.0 baseline).

> **The v1.7.16 milestone:** two Path A community calibrations, both
> ratchet-PASS (train strictly decreases, no per-region regression, no
> ihtiyat-unsafe drift). Morocco Dhuhr +5 extends the v1.5.0 Maghrib +5
> precedent (same scholarly-fixture-matching methodology, same Mawaqit-Morocco
> source, same Morocco-bounded scope). JAKIM Dhuhr +2 / Asr +1 applies the
> same pattern to Malaysia. **Critically, Aladhan + Diyanet per-source WMAE
> stayed exactly flat** — confirming the offsets are correctly region-bounded.
> See `autoresearch/logs/2026-05-03-17-30-iteration-1-morocco-dhuhr-path-a.md`

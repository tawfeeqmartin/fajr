...
|---|---|---:|---|---|
| Morocco | Canonical Morocco official-timetable calibration; single Morocco stance, no `MoroccoHabous` / `MoroccoMawaqit` aliases. Country default follows the official uniform city/region timetable stance: no auto city-registry elevation unless elevation is explicit. | A | Ministry of Habous current-month tables, Habous live checks, Mawaqit Morocco yearly corpus, PR #96 / issue #103, PR #180 public-path validation. | "Morocco official timetable calibration. Verify the mapped city if your local mosque follows a different Habous region; use explicit elevation only when your mosque intentionally follows observer-elevation correction." |
...
| Hong Kong / China border cells | Hong Kong keeps its Karachi-family local default; Shenzhen-side coordinates route to China fallback with Asia/Shanghai city provenance. | C | Hong Kong institutional default is documented; Shenzhen routing is geoBoundaries-backed geometry QA, but China-wide institutional validation remains sparse. | "Hong Kong and mainland China are separate defaults. Confirm local mosque or authority for mainland cities." |
